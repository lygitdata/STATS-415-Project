...
pdf(file = "manuscript_files/figure-latex/scattermatrix1.pdf", width = 8, height = 8)
...
pdf(file = "manuscript_files/figure-latex/scattermatrix2.pdf", width = 10, height = 10)
...
knitr::include_graphics("manuscript_files/figure-latex/scattermatrix2.pdf")
```
...
Given a training dataset $\{(x_i, y_i)\}_{i=1}^n$, where $x_i$ represents the predictors of the ith observation and $y_i$ is the corresponding label, XGBoost aims to learn an additive model $F(x)$ of the form:
...
With these hyperparameters, we used 10-fold cross-validation to get a test accuracy of 68.08163% and an AUC of 0.6845. The test accuracy is 0.08163% higher than that of multinomial regression model with lasso regularization, and the test AUC is `r 0.6845 - 0.6791` higher than that of multinomial model. These indicate that the XGBoost model is slightly better than multimonial regression model with lasso regularization when classifying the blood pressure levels.
...
xgboost.all = xgboost.train(colnames(train.X.dummy)[2:92])
...
importance.matrix = xgb.importance(colnames(train.X.dummy)[2:92], model = xgboost.all[[1]])
importance.matrix
```
...
pdf(file = "manuscript_files/figure-latex/importance_plot.pdf", width = 10, height = 3.5)
...
knitr::include_graphics("manuscript_files/figure-latex/importance_plot.pdf")
```
...
pdf(file = "manuscript_files/figure-latex/xgb_plots.pdf", width = 10, height = 4)
print(xgb.plots)
dev.off()
```
...
knitr::include_graphics("manuscript_files/figure-latex/xgb_plots.pdf")
```
...
\begin{table}[ht]
  \caption{XGBoost Model Top Predictors and Performance Metrics}
  \centering
...
This model exhibits higher increases in accuracy, with 0.65306% and 0.57143% improvements compared to the multinomial regression model with lasso regularization and the XGBoost model using all predictors, respectively. Moreover, it demonstrates a `r 0.6887414 - 0.6791` and `r 0.6887414 - 0.6845` increase in AUC compared to the multinomial regression model with lasso regularization and the XGBoost model using all predictors, respectively.
